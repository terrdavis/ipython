...
def backport_pr(branch, num, project="ipython/ipython"):
    ...
    if check.returncode:
        ...
        print(
            "edit {fname} until `cat {fname} | git apply --check` succeeds".format(
                **locals()
            )
        )
...
def already_backported(branch, since_tag=None):
    ...
    if since_tag is None:
        since_tag = (
            check_output(["git", "describe", branch, "--abbrev=0"])
            .decode("utf8")
            .strip()
        )
...
def should_backport(labels=None, milestone=None, project="ipython/ipython"):
    ...
    if labels is not None:
        issues = get_issues_list(project, labels=labels, state="closed", auth=True)
    else:
        milestone_id = get_milestone_id(project, milestone, auth=True)
        issues = get_issues_list(
            project, milestone=milestone_id, state="closed", auth=True
        )
...
if __name__ == "__main__":
    ...
    print(
        "DEPRECATE: backport_pr.py is deprecated and it is now recommended"
        "to install `ghpro` from PyPI.",
        file=sys.stderr,
    )
