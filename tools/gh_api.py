...
fake_username = "ipython_tools_%s" % socket.gethostname().replace(".", "_").replace(
    "-", "_"
)
...
def get_auth_token():
    ...
    if response.status_code == 401 and "required;" in response.headers.get(
        "X-GitHub-OTP", ""
    ):
...
def get_pull_request_files(project, num, auth=False):
    ...
    url = "https://api.github.com/repos/{project}/pulls/{num}/files".format(
        project=project, num=num
    )
    if auth:
        header = make_auth_header()
