...
def report(issues, show_urls=False):
    ...
    if show_urls:
        for i in issues:
            ...
            print(
                u"* :%s:`%d`: %s" % (role, i["number"], i["title"].replace(u"`", u"``"))
            )
    else:
        for i in issues:
...
if __name__ == "__main__":
    ...
    if milestone:
        print(
            "The full list can be seen `on GitHub <https://github.com/{project}/issues?q=milestone%3A{milestone}>`__".format(
