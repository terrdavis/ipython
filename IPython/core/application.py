...
if os.name == "nt":
    ...
    SYSTEM_CONFIG_DIRS = ["/usr/local/etc/ipython", "/etc/ipython"]
...
base_flags = dict(
    ...
    init=(
        {"BaseIPythonApplication": {"copy_config_files": True, "auto_create": True}},
...
class BaseIPythonApplication(Application):
    ...
    profile = Unicode(u"default", help="""The IPython profile to use.""").tag(
        config=True
    )
    ...
    def initialize_subcommand(self, subc, argv=None):
        if subc in self.deprecated_subcommands:
            ...
            self.log.warning(
                "You likely want to use `jupyter {sub}` in the "
