...
def test_get_ipython_dir_7():
    ...
    with modified_env({"IPYTHONDIR": os.path.join("~", "somewhere")}), patch.object(
        paths, "_writable_dir", return_value=True
    ):
        ipdir = paths.get_ipython_dir()
...
def test_get_ipython_cache_dir():
    with modified_env({"HOME": HOME_TEST_DIR}):
        if os.name == "posix" and sys.platform != "darwin":
            ...
            nt.assert_equal(paths.get_ipython_cache_dir(), paths.get_ipython_dir())
